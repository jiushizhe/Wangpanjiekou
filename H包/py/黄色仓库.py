...
pattern = r'https://[\w\d\.\-]+:\d+/\?u='

result = re.search(pattern, res.text)
if result:
    extracted_url = result.group()
    xurl=extracted_url+xurl+'/&p='
    res= requests.get(xurl,headers=headerx,allow_redirects=False)
    xurl = res.headers.get("Location")
